...
<body>
    ...
    <form action="./task_form_write.php" method="post">
...
<?php
    if (isset($_POST['nameTask']) && isset($_POST['desc']) && isset($_POST['dateTask']) && isset($_POST['state'])) {
        $task = new Task($_SESSION['usuari']->lastID(), $_POST['nameTask'], $_POST['desc'], $_POST['dateTask'], $_POST['state']);
        //echo $_SESSION['usuari']->lastID();
        if (!$_SESSION['usuari']->checkID($task)) {
            $_SESSION['usuari']->createTask($task);
        }
    }
?>